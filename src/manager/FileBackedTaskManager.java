...
import exceptions.*;
import logic.EpicDurationUpdater;
import logic.EpicStatusUpdater;
import task.*;
...
public class FileBackedTaskManager extends InMemoryTaskManager {
    ...
    public void loadFromFile(File file) {
                ...
                if (stringLine.endsWith(",")) {
                    ...
                    switch (type) {
                        ...
                        case "SUBTASK": {
                            ...
                            EpicStatusUpdater epicStatusVerify = new EpicStatusUpdater();
                            EpicDurationUpdater epicTimeVerify = new EpicDurationUpdater();
                            epics.get(((Subtask) task).getEpicId()).getSubtaskIds().add(task.getId());
                            epicStatusVerify.verifyStatus(epics.get(((Subtask) task).getEpicId()), subtasks);
                            epicTimeVerify.verifyTime(epics.get(((Subtask) task).getEpicId()), subtasks);
                            break;
                        }
                    }
        ...
        priorityTree.addAll(tasks.values());
        priorityTree.addAll(subtasks.values());
    }
